...
**TutuClawdbot** 是一个为您在电脑上运行的 *个人 AI 助手*。它可以通过您常用的社交软件（如：微信、Telegram、钉钉、WhatsApp 等）直接与您交流。它不仅能聊天，还能控制浏览器助手执行任务、管理您的长期记忆，甚至为您在实时画布（Canvas）上写代码。
...
2. **钉钉 (DingTalk) 流模式支持 (测试中)**
   ...
   - **部署极简**: 无需公网 IP 或反向代理，长连接直连。 *（期待您的反馈！）*
...
- **移动端节点**: iOS/Android 节点支持语音唤醒、相机联动和位置获取。
- **技能注册表 (ClawdHub)**: 助手可以自动搜索并拉取社区技能。

### 工作原理示意
```text
微信 / Telegram / 钉钉 / Slack / Discord ...
               │
               ▼
┌───────────────────────────────┐
│            Gateway            │
