...
**TutuClawdbot** 是一个基于 [Clawdbot](https://github.com/clawdbot/clawdbot) 的增强分支。它针对国内使用场景进行了优化，旨在提供更稳定的模型接入和更丰富的办公协作渠道。

---

### 🌟 核心增强特性

-   **OpenRouter 全面集成**: 无需为每个模型单独配置 API Key，通过 OpenRouter 接入 **Claude 4.5**, **Sonnet 4.5**, **Gemini 3**, **GPT-5** 等前沿模型。
-   **钉钉 (DingTalk) 原生支持**: 专为国内办公场景开发的 **钉钉 Stream 模式** 插件。无需公网 IP 和复杂的反向代理，即可实现长连接对话。
-   **Telegram 优化**: 完善的 TG 机器人集成，适合个人及海外场景使用。
-   **更简单的配置**: 预置了 OpenRouter 和钉钉的配置模板，开箱即用。

---

[原版项目文档](https://github.com/clawdbot/clawdbot) · [OpenRouter](https://openrouter.ai) · [钉钉开放平台](https://open.dingtalk.com)
...
### 2. 配置 AI 模型 (OpenRouter)
在 PowerShell 或 Bash 中设置 API Key：
...
#### 钉钉配置
```bash
# 进入 extensions/dingtalk 了解详细配置
pnpm clawdbot config set plugins.allow '["dingtalk"]'
```

#### Telegram 配置
```bash
pnpm clawdbot channels add --channel telegram --token "您的_BOT_TOKEN"
```
...
## 原版功能亮点
本分支完全兼容 Clawdbot 的所有核心能力：
- **[本地优先网关](https://docs.clawd.bot/gateway)** — 会话、渠道、工具和事件的统一控制平面。
- **[多渠道收集箱](https://docs.clawd.bot/channels)** — 支持 WhatsApp, Telegram, Slack, Discord 等。
- **[实时 Canvas](https://docs.clawd.bot/platforms/mac/canvas)** — 助手驱动的可视化工作区。
